...
public class BaseDAO extends HibernateDaoSupport {
	...
	/**
	 * 懒加载 返回所给id的实体类持久化实例，假定该实例存在。
	 * 该方法可能返回一个代理实例， 这个代理实例在非id的方法被访问的时候根据需要初始化。
	 * 如果查找的实例不存在，抛出异常。
	 * 
	 * @param <T>
	 * @param clazz
	 ...
	 * @param isAsc
	 * @return List<T>
	 * @see DetachedCriteria用于有大量的动态条件查询,是一种离线的查询 不需要session创建，
	 * 需Criteria正相反
	 */
	@SuppressWarnings("unchecked")
	 ...
	 * 。建议只在相同的事务内后续操作依赖于之前操作对数据库的改变时使用，
	 * 一般情况建议依赖于事务提交时的自动冲刷即可，无需手动调用此方法。
	 */
	public void flush() {
