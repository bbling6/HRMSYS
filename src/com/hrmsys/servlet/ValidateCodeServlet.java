...
public class ValidateCodeServlet extends HttpServlet {
	...
	public void HeightAndCode(int fontHeight,int codeY,int width,int height,int codeCount,int x) {
		// 宽度
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		HeightAndCode(fontHeight,codeY,width,height,codeCount,x);
		...
		g.setColor(Color.GRAY);
		for (int i = 0; i < 40; i++) {
			int x_temp = random.nextInt(width);
			int y = random.nextInt(height);
			int xl = random.nextInt(2);
			int yl = random.nextInt(2);
			g.drawLine(x_temp, y, x_temp + xl, y + yl);
		}
